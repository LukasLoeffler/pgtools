<template>
  <v-card dark class="ml-1" v-resize="updateTableDistance">
    <v-card-title>
      <v-row>
            <v-col cols="8"> 
            ...
            <v-col cols="4" class="text-right">
              <v-chip label small :color="getColor(  commandData.status )">{{   commandData.status }}</v-chip>
            </v-col>
          </v-row>
    </v-card-title>
    ...
    <v-card-text class="pa-1">
      ...
      <v-simple-table v-if="commandData.payload" ref="table" :height="`calc(100vh - ${tableDistanceTop}px)`">
        <thead>
          <tr>
