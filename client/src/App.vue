<template>
  <v-app id="app">
    <v-app-bar app color="blue-grey darken-2" clipped-left height="50">
      <v-app-bar-nav-icon>
        <v-btn color="white" icon>
          <v-icon>{{ currentIcon }}</v-icon>
        </v-btn>
      </v-app-bar-nav-icon>
      <v-toolbar-title style="color: white" class="pl-1">{{
          $route.name || "pgtools"
      }}</v-toolbar-title>
      <v-spacer></v-spacer>
      <ConnectionIndicator id="dbCon" :clickable="true" />
      <v-btn icon color="white" @click="settingsDrawer = !settingsDrawer">
        <v-icon>
          mdi-cog
        </v-icon>
      </v-btn>
    ...
    <v-navigation-drawer app :mini-variant="$vuetify.breakpoint.width <= 900" permanent
      :expand-on-hover="$vuetify.breakpoint.width > 900" clipped>
      <v-list>
        <v-list-item class="pl-2">
          ...
          <v-btn icon href="https://github.com/LukasLoeffler/pgtools" target="_blank">
    ...
    <v-navigation-drawer v-model="settingsDrawer" clipped temporary app right hide-overlay width="300">
      <v-toolbar height="50">
        <v-toolbar-title class="text-h6 font-weight-medium">Settings</v-toolbar-title>
...
export default {
  name: "App",
  components: { ConnectionIndicator, Darkmode, Config },
  data: () => ({
    mini: false,
    dark: false,
    drawer: null,
  ...
  computed: {
    currentIcon() {
      return this.sidebarElements.find(
